...
**Next session — wrap up the v1.6.0 deploy:**
- [ ] **Production smoke-test pending.** v1.6.0 is pushed (`cc2cf1d` + tag `v1.6.0` on `main`), `MAGIC_LINK_SECRET` is set in Railway, but at session close Railway's build queue was slow (known platform issue) and prod was still reporting v1.5.5 via `/openapi.json`. First check: `curl -s https://www.edgarwolf.com/openapi.json | python3 -c "import json,sys; print(json.load(sys.stdin)['info']['version'])"` — when it returns `1.6.0`, run the prod smoke test (see below).
- [ ] **Smoke test once deployed:** `/health` ok, `/auth/whoami` returns standard tier without cookie, magic-link round-trip from jason@edgarwolf.com (request → email → click → `/?signed_in=1` with cookie → watchlist GET succeeds), and `/auth/request` with an unknown email returns the same `{ok: true}` body.
- [ ] Confirm `SEC_REQUIRE_EXPLICIT_USER_AGENT=true` and `SEC_USER_AGENT` are set in Railway (env var check, no code change).
- [ ] **Open design question:** is weekly `MAGIC_LINK_SECRET` rotation the right security control? Rotation defends against a leaked secret, but if the secret never leaves Railway env vars it has diminishing value. Alternative controls worth considering: short session TTL with server-side session table for revocation; audit logging of `/auth/verified` events; Sentry alerts on `auth.py` `RuntimeError` (catches a missing env var fast); rate-limit anomaly alerts on `/auth/*`. Settle this before doing more rotation work.
- [ ] **v1.6.1 candidate — dual-key support in `auth.py`.** Add `MAGIC_LINK_SECRET_PREVIOUS` fallback for verify-only so rotation doesn't invalidate existing sessions. ~10 lines in `verify_token`. Lets weekly rotation cost zero user-facing friction once there are paying users.
- [ ] Postman collection regeneration — still references `/subscription/restore`, `/subscription/status*`, and `X-Customer-Id`. Separate commit, low priority since QA wasn't blocking the push.
...
_Last updated: May 20, 2026 — v1.6.0 pushed, deploy queued_
